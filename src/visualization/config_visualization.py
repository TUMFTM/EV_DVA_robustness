...
def instantiate_matplotlib_parameters():
    #regular font
    font_path_regular = os.path.join(GeneralConfig.path2data_font.value, 'STIXTwoText-Regular.ttf')
    font_manager.fontManager.addfont(font_path_regular)
    prop = font_manager.FontProperties(fname=font_path_regular)
    # bold font
    font_path_bold = os.path.join(GeneralConfig.path2data_font.value, 'STIXTwoText-Bold.ttf')
    font_manager.fontManager.addfont(font_path_bold)
    prop = font_manager.FontProperties(fname=font_path_bold)
    # custom prop
    # plt.rcParams.update({'font.family': 'serif'}) #or sans-serif
    #plt.rcParams.update({'font.sans-serif': prop.get_name()})
    #plt.rcParams.update({'font.serif': prop.get_name()})
    #plt.rcParams.update({'font.cursive': prop.get_name()})
    #plt.rcParams.update({'font.fantasy': prop.get_name()})
    #plt.rcParams.update({'font.monospace': prop.get_name()})
    ...
    #plt.rcParams.update({'mathtext.it':'STIX Two Text:regular'})
...
class VisualizationConfig(enum.Enum):
    ...
    # for A4 TUM Dissertations the figures are 0.9*columnswidth
    # for paper 1*columnswidth
    factor_col_width_figure = 0.9
