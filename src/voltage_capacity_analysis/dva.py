...
import sys
import os
sys.path.append(os.path.join(os.getcwd().partition('chargecom')[0],"chargecom"))
...
class DVA(VCABase,FilterMethods):
    ...
    def get_dva(self, time, capacity, voltage, crop_cap_Ah=(0,165), absolute=False,
        ...
        return np.array(masked_dva), np.array(masked_capacity)
    ...
    def get_graphite_peak_location(self,capa,dva,thres1,thres2,n_parallel=1):
        ...
        return capa[(capa > thres1) & (dva == max_dva)] / n_parallel
...
if __name__ == "__main__":
    ...
    path2file = os.path.join(GeneralConfig.path2data_test.value, "df_test_pOCV.pkl")
    df = pd.read_pickle(path2file)

    dva_extractor = DVA()
    df["Q"] = dva_extractor.calculate_capacity_signal(df["time_h"], df["hv_battery_current"], initial=0)
    dva = dva_extractor.get_normalized_dva(time=df["time_h"],
                                           capacity=df["Q"],
                                           voltage=df["hv_battery_voltage"],
                                           crop_cap_Ah=(10,165),
                                           absolute=False)

    df["cell_voltage_1"] = df["hv_battery_voltage"]
    df["cell_voltage_2"] = df["hv_battery_voltage"]
    df_dva_cells, df_cap_cell = dva_extractor.get_cell_dva_from_pack(time = df["time_h"],
                                                                     capacity = df["Q"],
                                                                     df_cell_voltages = df)
    print("DVA Extracted")
